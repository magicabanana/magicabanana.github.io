...
async function mergeImages() {
    ...
    const finalW = parseInt(finalWidthInput.value) || 0;
    const finalH = parseInt(finalHeightInput.value) || 0;
    ...
    let calW = finalW > 0 ? finalW : totalWidth;
    let calH = finalH > 0 ? finalH : totalHeight;

    // 2의 거듭제곱으로 변환하는 함수
    const getNextPowerOf2 = (n) => Math.pow(2, Math.ceil(Math.log2(n)));

    // 최종 크기는 무조건 2의 거듭제곱
    calW = getNextPowerOf2(calW);
    calH = getNextPowerOf2(calH);
